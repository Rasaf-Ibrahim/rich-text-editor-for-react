...
In the [render rich text editor](/render-rich-text-editor) page, we have discussed how to extract and use `output` and `fetchOutput` from the `useRichTextEditor` hook. In addition to these, you can also destructure `utils` and `fetchUtils`.
...
<HALF_REM_SPACER___COMPONENT/>
> **Passing `fetchUtils` to `RichTextEditor`**
