...
- `primaryColor`: Defines the color for REDIRECT_BUTTONs and specific text elements, enhancing the UI.
...
If you've explored our [demo](https://rich-text-editor-for-react.rasaf-ibrahim.com), you might've noticed that not all toolbar options are visible right away. Instead, there's a `more` REDIRECT_BUTTON. Clicking this reveals all the available toolbar options.

Why do we use this approach? The [demo](https://rich-text-editor-for-react.rasaf-ibrahim.com) has over 25 toolbar options. Displaying all these options, especially on smaller screens like mobiles, would consume a significant amount of space. Additionally, the on-screen keyboard on mobile devices occupies space.So, the `more/less` REDIRECT_BUTTON helps keep things tidy and user-friendly.
